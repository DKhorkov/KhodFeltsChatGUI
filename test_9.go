...
func (cw *ChatWindow) setupScrollListener() {
	// Получаем доступ к scroller через рефлексию или сохраняем при создании
	// В Fyne мы можем получить scroller, если обернем List в Scroll контейнер
	// Но List уже имеет встроенный scroller

	// Запускаем горутину для периодической проверки позиции скролла
	go func() {
		for {
			time.Sleep(200 * time.Millisecond)

			if cw.currentChat == nil || cw.loadingMore || !cw.hasMore {
				continue
			}

			// Получаем текущую позицию скролла
			// В Fyne нет прямого доступа к позиции скролла List,
			// но мы можем использовать метод GetScrollOffset если он доступен
			// Вместо этого будем использовать приближение через видимые элементы

			// Проверяем, видим ли мы первые несколько элементов
			cw.checkScrollPosition()
		}
	}()
}

func (cw *ChatWindow) checkScrollPosition() {
	// Если мы у начала списка (видим первые несколько элементов)
	// и есть еще сообщения для загрузки - загружаем
	if len(cw.messages) > 0 && !cw.loadingMore && cw.hasMore {
		// В Fyne мы не можем точно определить видимые элементы,
		// поэтому используем приближение: если мы недавно обновляли список
		// и пользователь скроллит вверх, вероятно, он хочет увидеть старые сообщения

		// Более точный способ - использовать событие скролла,
		// но в Fyne его нужно реализовать через кастомный контейнер

		// Для демонстрации будем загружать следующие сообщения,
		// когда пользователь достигает начала списка
		// Это можно определить по тому, что SelectedItem близок к 0
	}
}

// Упрощенная версия - загружаем по команде с кнопки
// Добавим кнопку "Загрузить историю" для тестирования
func (cw *ChatWindow) addLoadMoreButton() fyne.CanvasObject {
	return widget.NewButton("Загрузить историю", func() {
		cw.loadMoreMessages()
	})
}
...
func (cw *ChatWindow) getScrollPosition() float32 {
	// В Fyne мы можем получить позицию скролла через метод GetScrollOffset
	// если он доступен. В противном случае возвращаем приблизительное значение
	// на основе выбранного элемента
	if len(cw.messages) > 0 {
		// Возвращаем приблизительную позицию (0 - вверху, 1 - внизу)
		// Это упрощение, в реальном приложении нужно использовать реальный скролл
		return 0.5 // Средняя позиция по умолчанию
	}
	return 0
}

func (cw *ChatWindow) restoreScrollPosition(oldPos float32, newMessagesCount int) {
	// Восстанавливаем позицию скролла после добавления новых сообщений
	// В Fyne это можно сделать через ScrollToPosition
	if len(cw.messages) > 0 {
		// Скроллим к тому месту, где были новые сообщения
		// newMessagesCount - количество добавленных старых сообщений
		cw.messagesList.ScrollTo(newMessagesCount)
	}
}
