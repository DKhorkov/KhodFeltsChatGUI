...
//	currentChat  *Chat
//	chats        []Chat
//	messages     []Message // Храним в том же порядке, что от сервера: старые (индекс 0) -> новые (последний индекс)
...
//			}
//
//			// ВАЖНО: Сервер возвращает сообщения от старых к новым
//			// id=0 - это первый элемент в списке (самый верхний) -> самое старое сообщение
//			// id=len-1 - последний элемент в списке (самый нижний) -> самое новое сообщение
//			msg := cw.messages[id]
//
//			container_ := item.(*fyne.Container)
...
//	messagesMu.Lock()
//	if offset == 0 {
//		// Первая загрузка - сообщения от сервера уже в правильном порядке:
//		// старые сначала, новые потом
//		slices.Reverse(messages)
//		cw.messages = messages
//	} else {
//		// Подгрузка старых сообщений - они СТАРШЕ, чем текущие, поэтому вставляем В НАЧАЛО
...
//	fyne.Do(func() {
//		cw.messagesList.Refresh()
//		// При первой загрузке скроллим к последнему сообщению (вниз)
...
//	}
//
//	// Создаем сообщение для локального отображения
//	localMsg := Message{
//		ID:        uint64(time.Now().UnixNano()), // Временный ID
...
//	}
//
//	// Добавляем в КОНЕЦ слайса (новое сообщение должно быть внизу)
//	messagesMu.Lock()
...
//
//	// Обновляем список и скроллим вниз к новому сообщению
//	cw.messagesList.Refresh()
//	cw.messagesList.ScrollToBottom()
//
...
//	// Игнорируем свои сообщения (они уже добавлены локально)
//	if msg.Sender.ID == currentUser.ID {
//		return
//	}
//
