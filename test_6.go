...
//			}
//
//			// ВАЖНО: id=0 - это первый элемент в списке (самый верхний)
//			// Мы хотим, чтобы самые старые сообщения были вверху (индекс 0)
//			// Поэтому берем сообщение прямо по индексу, без реверса
//			msg := cw.messages[id]
//
//			container_ := item.(*fyne.Container)
...
//	messagesMu.Lock()
//	if offset == 0 {
//		// Первая загрузка - сообщения должны быть в хронологическом порядке
//		// (старые сначала, новые потом)
//		cw.messages = messages
//	} else {
//		// Подгрузка старых сообщений - добавляем их в НАЧАЛО
//		// потому что они старше тех, что уже есть
