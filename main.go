...
//			}
//
//			if wsMsg.Type == "message" && wsMsg.Message != nil {
//				onMessage(wsMsg.Message)
//			}
//		}
//	}()
//
...
//		Type:   "message",
//		ChatID: chatID,
//		Data:   json.RawMessage(fmt.Sprintf(`{"text":"%s"}`, text)),
//	}
//
...
//				return
//			}
//			msg := cw.messages[len(cw.messages)-1-id] // Обратный порядок (новые снизу)
//
...
//	)
//
//	// Добавляем скролл для подгрузки старых сообщений
//	cw.messagesList.OnSelected = nil
//
//	// Поле ввода
...
//	}
//
//	// Обновляем статус прочтения в списке чатов
//	for i, chat := range cw.chats {
//		if chat.ID == msg.ChatID && msg.Sender.ID != currentUser.ID {
...
//	if cw.currentChat == nil || msg.ChatID != cw.currentChat.ID {
//		showNotification(cw.app, "Новое сообщение", fmt.Sprintf("%s: %s", msg.Sender.Username, msg.Text))
//	}
//}
//
...
//	fyne.Do(func() {
//		dialog.ShowInformation(title, message, app.NewWindow("Уведомление"))
//	})
//}
//
...
//		go func() {
//			users, err := searchUsers(cw.token, searchEntry.Text, 20, 0)
//			if err != nil {
//				return
//			}
//
...
//
//		go func() {
//			_, err := createChat(cw.token, typeEntry.Selected, memberIDs)
//			if err != nil {
...
//	return strings.ReplaceAll(urlQueryEscape(query), "+", "%20")
//}
