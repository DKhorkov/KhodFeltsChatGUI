...
//	loadingMore     bool
//	pendingMessages map[uint64][]Message // Локальные сообщения, ожидающие подтверждения
...
//		func() int {
//			cw.pendingMu.RLock()
//			defer cw.pendingMu.RUnlock()
//
//			// Общее количество = загруженные + ожидающие для текущего чата
//			total := len(cw.messages)
//			if cw.currentChat != nil {
//				total += len(cw.pendingMessages[cw.currentChat.ID])
//			}
//			return total
//		},
//		func() fyne.CanvasObject {
...
//				container.NewBorder(nil, nil, sender, container.NewHBox(statusIcon, time_)),
//				message,
//				widget.NewSeparator(),
//			)
//		},
...
//
//			cw.pendingMu.RLock()
//			pendingCount := len(cw.pendingMessages[cw.currentChat.ID])
//			totalCount := len(cw.messages) + pendingCount
//			cw.pendingMu.RUnlock()
//
//			if id >= totalCount {
//				return
//			}
...
//			rightPanel := border.Objects[1].(*fyne.Container)
//			sender := border.Objects[0].(*widget.Label)
//			statusIcon := rightPanel.Objects[0].(*widget.Icon)
//			time_ := rightPanel.Objects[1].(*widget.Label)
//			message := container_.Objects[1].(*widget.Label)
//
//			// Определяем, это загруженное сообщение или ожидающее
//			if id < pendingCount {
//				// Это ожидающее сообщение (в обратном порядке)
//				cw.pendingMu.RLock()
//				pendingMsgs := cw.pendingMessages[cw.currentChat.ID]
//				msg := pendingMsgs[pendingCount-1-id] // Обратный порядок
//				cw.pendingMu.RUnlock()
//
//				sender.SetText("Вы")
//				message.SetText(msg.Text)
//				time_.SetText("отправка...")
//				statusIcon.SetResource(theme.InfoIcon())
//			} else {
//				// Это загруженное сообщение
//				msgIdx := id - pendingCount
//				msg := cw.messages[len(cw.messages)-1-msgIdx] // Обратный порядок
//
...
//				statusIcon.SetResource(nil) // Нет иконки для доставленных
//			}
//		},
//	)
//
...
//	cw.pendingMu.Lock()
//	cw.pendingMessages[cw.currentChat.ID] = append(cw.pendingMessages[cw.currentChat.ID], pendingMsg)
//	cw.pendingMu.Unlock()
//
...
//	if err != nil {
//		// Если ошибка отправки - помечаем сообщение как ошибочное
//		cw.markPendingMessageAsFailed(cw.currentChat.ID, pendingMsg)
//		cw.showError("Ошибка отправки", err)
//		return
//	}
//}
//
//func (cw *ChatWindow) markPendingMessageAsFailed(chatID uint64, failedMsg Message) {
//	cw.pendingMu.Lock()
//	defer cw.pendingMu.Unlock()
//
//	pending := cw.pendingMessages[chatID]
//	for i, msg := range pending {
//		if msg.Text == failedMsg.Text && msg.Sender.ID == failedMsg.Sender.ID {
//			// Можно изменить сообщение, но Fyne List не поддерживает обновление отдельных элементов
//			// Поэтому просто удалим его и покажем ошибку
//			cw.pendingMessages[chatID] = append(pending[:i], pending[i+1:]...)
//			break
//		}
//	}
//
//	fyne.Do(func() {
//		cw.messagesList.Refresh()
//		dialog.ShowError(fmt.Errorf("Сообщение не доставлено"), cw.window)
//	})
//}
//
//func (cw *ChatWindow) onNewMessage(msg *Message) {
//	// Удаляем это сообщение из ожидающих (если это наше сообщение)
//	if msg.Sender.ID == currentUser.ID {
//		cw.pendingMu.Lock()
//		pending := cw.pendingMessages[msg.ChatID]
//		for i, pendingMsg := range pending {
//			// Сравниваем по тексту и времени (грубо, но для демо пойдет)
//			if pendingMsg.Text == msg.Text &&
//				pendingMsg.CreatedAt.Sub(msg.CreatedAt) < time.Second {
//				cw.pendingMessages[msg.ChatID] = append(pending[:i], pending[i+1:]...)
//				break
//			}
//		}
//		cw.pendingMu.Unlock()
