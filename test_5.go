...
func showMainChatWindow(myApp fyne.App, token string) {
	...
	cw.messagesList = widget.NewList(
		...
		func(id widget.ListItemID, item fyne.CanvasObject) {
			...
			// Здесь важно: id=0 - это первый элемент в списке (самый верхний)
			// Мы хотим, чтобы самые старые сообщения были сверху, новые - снизу
			msg := cw.messages[id] // Просто берем по индексу, без реверса!
...
func (cw *ChatWindow) loadMessages(chatID uint64, offset int) {
	...
	messagesMu.Lock()
	if offset == 0 {
		// Первая загрузка - сообщения уже должны быть от сервера в хронологическом порядке
		// (старые сначала, новые потом)
		cw.messages = messages
	} else {
		// Подгрузка старых сообщений - добавляем их в начало
	...
	fyne.Do(func() {
		...
		if offset == 0 && len(cw.messages) > 0 {
			...
			// При подгрузке старых сообщений скроллим к тому месту, где были
			// TODO: можно сохранить позицию
		}
	})
...
func (cw *ChatWindow) sendMessage() {
	...
	// Добавляем в КОНЕЦ слайса (новые сообщения должны быть внизу)
	messagesMu.Lock()
	...
	// Важно: сначала обновляем список, потом скроллим
	cw.messagesList.Refresh()
	cw.messagesList.ScrollToBottom()
